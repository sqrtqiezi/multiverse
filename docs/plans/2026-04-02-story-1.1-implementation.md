...
import * as path from 'node:path';
import * as os from 'node:os';
import type { CredentialPath } from '@multiverse/types';
...
git commit -m "feat: implement CredentialResolver for selective credential mounting"
```
...
export async function startCommand(): Promise<void> {
  ...
  const credentialPaths = await credentialResolver.resolveCredentialPaths();
  ...
  console.log(`✓ Found ${credentialPaths.length} credential file(s)\n`);
  ...
  const config: ContainerConfig = {
    image: imageTag,
    volumes: [workspaceMount, ...credentialPaths],
    workDir: '/workspace',
    autoRemove: true,
  };
